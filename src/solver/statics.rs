...
pub struct GeneralizedAlphaSolver {
    ...
    state: State,
    state_next: State,
    q_delta: Matrix6xX<f64>, // delta displacement+rotation
    lambda: DVector<f64>,    // constraints
...
impl GeneralizedAlphaSolver {
    pub fn new(
        ...
        GeneralizedAlphaSolver {
            ...
            state_next: State::new(num_state_nodes),
            q_delta: Matrix6xX::zeros(num_state_nodes * 6),
            lambda: DVector::zeros(num_constraint_nodes * 6),
    ...
    pub fn update_next_displacement(&mut self) {
        ...
        let u = self.state.q.fixed_rows::<3>(0);
        let du = self.q_delta.fixed_rows::<3>(0);
        let mut un = self.state_next.q.fixed_rows_mut::<3>(0);
        un.copy_from(&(&du * self.h + &u));

        // Update rotations
        let r = self.state.q.fixed_rows::<4>(3);
        let dr = self.q_delta.fixed_rows::<3>(3);
        let mut rn = self.state_next.q.fixed_rows_mut::<4>(3);
        for i in 0..r.ncols() {
            // Get current rotation as a quaternion
            let R = Vector4::from(r.column(i)).as_unit_quaternion();

            // Get change in rotation as quaternion
            let dR = UnitQuaternion::from_scaled_axis(self.h * dr.column(i));
    ...
    pub fn solve_time_step(&mut self, elem: &mut Element) -> Option<usize> {
        ...
        for i in 0..10 {
            // Use q_delta to predict state_next.q
            self.update_next_displacement();

            // Apply new state to element
            let u: Matrix3xX<f64> = Matrix3xX::from(self.state_next.q.fixed_rows::<3>(0));
            let r: Matrix4xX<f64> = Matrix4xX::from(self.state_next.q.fixed_rows::<4>(3));
            elem.displace(&u, &r);
            ...
            let R_C: DVector<f64> = B.transpose() * &self.lambda;
            ...
            let x_delta = x.rows(0, num_node_dofs);
            let lambda_delta = x.rows(num_node_dofs, num_constraint_dofs);

            // update delta q
            self.q_delta -= Matrix6xX::from_column_slice((x_delta / self.h).as_slice());
...
mod tests {
    ...
    use nalgebra::{
        DMatrix, DVector, Matrix3, Matrix3xX, Matrix4xX, Matrix6, UnitQuaternion, Vector3, Vector4,
    };
    ...
    use crate::{
        ...
        solver::statics::GeneralizedAlphaSolver,
    };
