...
fn test_cantilever_beam_with_with_sin_load() {
    ...
    while solver.state.t <= tf {
        ...
        match solver.step(&mut elem) {
            None => {
                print!(
                    "{:.3}s: failed to converge \n",
                    solver.state.t + solver.step_config.h
                );
                break;
            }
            Some(residual_norms) => {
                ...
                file.write_fmt(format_args!("{:?}", solver.state.t))
                    .expect("fail");
                for &v in solver.state.displacement(&solver.step_config).iter() {
